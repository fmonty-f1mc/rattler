"""
Golf Tournament Planner - Single-file Flask app
...
"""
"""
Golf Tournament Planner with Leaderboard Link
...
class Player(db.Model):
    ...
    entries = db.relationship('TournamentPlayer', back_populates='player', cascade='all, delete-orphan')

class TournamentPlayer(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    tournament_id = db.Column(db.Integer, db.ForeignKey('tournament.id'))
    player_id = db.Column(db.Integer, db.ForeignKey('player.id'))
    tee_time_order = db.Column(db.Integer, default=0)
    group_number = db.Column(db.Integer, default=0)
    ...
    def update_scores(self, scores, holes):
        self.hole_scores = ','.join(str(s) for s in scores)
        self.gross_score = sum(scores)
        strokes_allocated = floor(self.player.handicap / holes)
...
base_tpl = """
<!doctype html>
<title>Golf Tournament Planner</title>
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/water.css@2/out/water.css">
<header>
  <h1>Golf Tournament Planner</h1>
  <nav>
...
def create_tournament():
    if request.method == 'POST':
        ...
        t = Tournament(name=name, date=date, holes=holes)
        db.session.add(t)
        db.session.commit()
...
def view_tournament(tid):
    ...
    if request.method == 'POST':
        ...
        if TournamentPlayer.query.filter_by(tournament_id=tid, player_id=pid).first():
...
def score_entry(tid, eid):
    ...
    {% endblock %}
    """
    return render_template_string(tpl, e=e, scores=scores, holes=holes)
